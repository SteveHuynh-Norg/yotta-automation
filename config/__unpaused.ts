...
/** Pause reason for a page, if the global or a per-host window is still open. */
function pauseReasonFor(url: string): string | undefined {
  if (GLOBAL_PAUSE && pauseIsOpen(GLOBAL_PAUSE.until)) {
    return `All form submissions paused until ${GLOBAL_PAUSE.until} — ${GLOBAL_PAUSE.reason}.`;
  }
